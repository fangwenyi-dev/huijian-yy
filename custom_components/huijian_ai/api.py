...
def _extract_automation_info(automation: dict) -> dict:
    """Extract automation info for display."""
    ...
    if above is not None:
        condition_parts.append(f"> {above}")
    if below is not None:
        condition_parts.append(f"< {below}")

    action_summaries = []
    for a in actions:
        intent_name = a.get("name") or a.get("intent", "Unknown")
        params = a.get("parameters") or a.get("params", {})
        target = params.get("target", [])
        parts = []
        for t in target:
            area = t.get("area", "")
            ...
            for d in devices:
                ...
                if area:
                    parts.append(f"{area} {name or '/'.join(domains)}")
                elif name:
                    parts.append(name)
                else:
                    parts.append('/'.join(domains))
        action_summaries.append(f"{intent_name} ({', '.join(parts) if parts else '无目标'})")

    return {
        "automation_id": automation.get("automation_id"),
        "trigger_entity": entity_id,
        "trigger_condition": "、".join(condition_parts) if condition_parts else "无",
        "action_count": len(actions),
        "action_summaries": action_summaries,
...
class AutomationsListView(HomeAssistantView):
    ...
    async def get(self, request: web.Request):
        ...
        try:
            ...
            automation_list = [_extract_automation_info(a) for a in automations]
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-title { font-size: 16px; font-weight: 600; color: #1976d2; }
...
<body>
    <div class="container">
        ...
        <div class="nav">
            <a href="/huijian-ai/voice-scenes/manage">查看语音场景 &raquo;</a>
        </div>
    ...
    <script>
        ...
        async function loadAutomations() {
            ...
            try {
                ...
                content.innerHTML = automations.map(a => `
                    <div class="card" id="auto-${a.automation_id}">
                        <div class="card-header">
                            <span class="card-title">${escapeHtml(a.trigger_entity)}</span>
                            <div>
                                <span class="tag">${escapeHtml(a.trigger_condition)}</span>
                                <button class="delete-btn" onclick="deleteAutomation('${a.automation_id}', '${escapeHtml(a.trigger_entity)}')">删除</button>
                            </div>
                        </div>
                        <div class="info">
                        ...
                        <div class="actions-box">
                            ...
                            ${(a.action_summaries || []).map(s => `<div class="action-item">- ${escapeHtml(s)}</div>`).join('')}
                        </div>
                    </div>
                `).join('');
        ...
        async function deleteAutomation(automationId, triggerEntity) {
            if (!confirm('确定要删除传感器自动化 "' + triggerEntity + '" 吗？')) {
                return;
            }
            ...
            try {
                const response = await fetch(API_BASE + '/' + automationId, {
                    method: 'DELETE'
                });
                const data = await response.json();
